...
void Game::deinit() { this->m_ghosts.clear(); }
...
void Game::processEvents(std::vector<ANAL::Event> &ev)
{
    ...
    if (this->_pacmanmap[new_y][new_x] == '#' || this->_pacmanmap[new_y][new_x] == '_')
        return;
...
void Game::compute(ANAL::IArcade &arcade)
{
    ...
    for (Ghost &e : this->m_ghosts)
    {
        if (e.getPos().x == this->m_player.getPos().x && e.getPos().y == this->m_player.getPos().y)
        {
            if (this->m_player.getState() == Player::HIGH)
            {
            ...
            if (this->m_player.getState() != Player::HIGH && e.getState() != Ghost::DEAD)
    ...
    for (Ghost &g : this->m_ghosts)
    {
        if (g.getState() != Ghost::DEAD)
        {
            ...
            if (this->_pacmanmap[pos.y][pos.x - 1 - 1] != '#' && this->_pacmanmap[pos.y][pos.x - 1 - 1] != '_')
